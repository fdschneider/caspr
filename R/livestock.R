...
livestock$update <- function(x_old, parms, subs = 12) {
  ...
  # 1 - setting time-step parameters
    ...
    q_one_one <- count(x_old, "1")/4  # count local density of occupied fields for each cell
  
  # 2 - drawing random numbers
  rnum <- runif(x_old$dim[1]*x_old$dim[2]) # one random number between 0 and 1 for each cell
  
  # 3 - setting transition probabilities
  growth <- with(parms, (r * (b + (1-b)*f*q_one_one) * rho_one^(1 + alpha) * ( 1 - (rho_one / (K * (1-c*q_one_one) ))) / (1 - rho_one))  *1/subs)  # recolonisation rates of all cells 
